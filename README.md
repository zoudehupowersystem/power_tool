...
### 7.1 短路电流（系统+线路串联，故障在线路末端）

该页默认计算模型为：**系统等值阻抗 + 线路阻抗串联**，故障点位于线路末端。

#### 输入方式

1. 系统侧参数：$U$、系统短路容量 $S_{sc}$、系统 $X/R$。
2. 线路参数：长度 $l$、正序参数 $R_1,X_1$（$\Omega/km$）、零序参数 $R_0,X_0$（$\Omega/km$）。
3. 故障参数：故障类型（中文）、过渡电阻 $R_f$。
4. 中性点接地方式：
   - 直接接地
   - 中性点不接地
   - 经消弧线圈接地（输入 $X_n$）
...
程序采用对称分量法构建复合序网，先求序电流 $I_1,I_2,I_0$，再恢复相电流：
...
1&a&a^2
\end{bmatrix}
\begin{bmatrix}I_0\\I_1\\I_2\end{bmatrix},\quad a=e^{j120^\circ}
$$
...
- $I_1,I_2,I_0$ 相量
- $I_a,I_b,I_c$ 相量
- 开断校核电流 $I_{break}=\max(|I_a|,|I_b|,|I_c|)$
- 直流偏置时间常数 $\tau=\dfrac{X_{eq}}{\omega R_{eq}}$
...
且波形含**交流分量 + 指数衰减直流偏置**，用于更贴近开断与保护场景的工程观察。

#### 断路器匹配

若填写断路器额定开断电流 $I_{k,rated}$，程序判定是否满足：

$$
I_{k,rated} \ge I_{break}
$$
