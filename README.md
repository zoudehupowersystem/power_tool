...
用途：给定系统电压 $U$ 与短路容量 $S_{sc}$，快速估算三相对称短路电流：

$$
I_{sc}=\frac{S_{sc}}{\sqrt{3}U}
$$

并计算等值 Thevenin 阻抗：

$$
Z_{th}=\frac{U^2}{S_{sc}}
$$

若输入断路器额定开断电流，还会给出是否满足 $I_{k,rated}\ge I_{sc}$ 的匹配判断。

### 7.2 功率因数补偿

用途：把负荷功率因数从 $\cos\varphi_1$ 提高到 $\cos\varphi_2$，快速估算所需并联电容器补偿容量：

$$
Q_c=P(\tan\varphi_1-\tan\varphi_2)
$$

当提供系统电压时，程序还会估算补偿前后线电流与 50Hz 星形等效每相电容值，便于初步选型。
