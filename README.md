...
输入项包括：$U$、$Z_1=R_1+jX_1$、$Z_2=R_2+jX_2$、$Z_0=R_0+jX_0$、过渡电阻 $R_f$，并可选输入断路器开断电流额定值。

程序采用对称分量法构建复合序网，求解故障点序电流 $I_1,I_2,I_0$，再恢复相电流：
...
> 说明：当前波形为工频交流分量模拟，便于工程对比与保护整定初算；未叠加直流偏置和高频暂态分量。

断路器开断能力匹配依据：

- 取三相电流 RMS 幅值最大值作为 $I_{break}$
- 校核是否满足 $I_{k,rated} \ge I_{break}$
